...
import javax.swing.*;
 ...
 public class PanelJuego extends JPanel implements KeyListener{
     ...
     private   int  nivel=0,contJefe=0,puntos=0,velocidad=3 ,contDisparos=0,resistencias[]=new int[this.tamanoHilera],contR=0;
     private final String   hilera[]={"A","B","D","E","C","Basic"};//Nombre de las hileras
     public String   listaActual="",listaSiguiente,nombreJug;
     public final int  contDisparosHileraD[]=new int[5], tamanoHilera=5;//vector que guarda disparos de cada jugador en su posicion
...
/**
 *Esta función se encarga de crear todos los componentes del juego , como la hilera de invasores que se crea aleatoriamente, la nave, y también se asigna aleatoriamente un jefe en cualquier posición.     
 */
 ...
 //////////se crea la nave
  ...
  nave=new Nave(c,c1,c2,Color.red);  elementos.add(nave); 
   /////////////////////////////////////////crea hilera actual y siguiente  aleatoriamente
 listaActual=listaSiguiente;//Lista Actual
 listaSiguiente=hilera[(int)(Math.random()*(hilera.length) + 0 )];//Lista Siguiente
          if(listaActual.equals("Basic")){invasores =new ListaBasic(); }
         if(listaActual.equals("A")){invasores =new ListaA(); }
         if(listaActual.equals("B")){invasores =new ListaB(); }
         if(listaActual.equals("C")){invasores =new ListaC(); }
         if(listaActual.equals("D")){invasores =new ListaD(); }
         if(listaActual.equals("E")){invasores =new ListaD(); }
     
   ////////////////////////////////////////////Asigna aleatoriamente un jefe a la hilera
         boolean hayJefe=false;//es falso porque no tenemos jeef aun
         for(int i=1;i<=tamanoHilera;i++){
              String j=i+"i";//Asigna id "i" al jefe pero el jefe debe tener id "j"!!!!!!!!!
               int id= (int)(Math.random()*2) ;
           if(id==0 &&hayJefe==false&&!listaActual.equals("Basic")){ //Si id==0 , el id es "j" Osea es el jefe
        j=i+"j";hayJefe=true; }
        ...
        j=i+"i"+""; }
         ...
         /////////////////////////////////////////////////////////////////////Agrega invasores a la hilera
           nh=new NombreHilera(0,20,40,"Hilera:"+listaActual,Color.RED);//se crea etiqueta del nombre de hilera
           ...
           if(nh.hilera.equals("Hilera:E")){
            ...
            invasores.insertar(inv);  elementos.add(inv);}
           if(nh.hilera.equals("Hilera:D")){ 
               while(true){contR=0;
                        int a= (int)(Math.random()*this.tamanoHilera) +1;
                        resistencias[cont2]=a;
                        
                        for(int x=0;x<this.tamanoHilera;x++){
                        if(a!=resistencias[x]){contR++;}}
                        
                   if(contR==this.tamanoHilera-1){ cont2++;
                Invasor inv=new Invasor(new Coordenada((i+2)*100,40),40,Color.BLUE,j,velocidad,a );
               invasores.insertar(inv);elementos.add(inv);break;}
        ...
        invasores.imprimir();
          ...
          }      if(hayJefe==false&&nh.hilera.equals("Hilera:B")){invasores.raiz.info.id=1+"j"+"";}// se asigna jefe por defecto si no hay
       invasores.imprimir();  
////////////////////////////////////ordena hilera por resistenci

   /* int n = this.tamañoHilera;  
        float temp = 0;  
         for(int i=0; i < n; i++){  
                 for(int j=1; j < (n-i); j++){  
                          if(arr[j-1] <  arr[j]){  
 ...
 /**
 *Basicamente este método se encarga de pintar los componentes en el panel del juego, es un método @Override , quiere decir que se implementa de la interfaz Dibujable y se llama automáticamente.     
 */
    @Override
  ...
  */
    @Override
   public void update(Graphics g){paint(g);}//actualiza pantalla.......
        ...
        while(true){ 
            try{ 
               ...
               Nodo  actual=invasores.raiz;
                  ...
                  while(actual!=null){
                	  if(actual.info.getY()==nave.getY()) {JOptionPane.showMessageDialog(null, "Fin del juego");
                	  System.exit(0);}
                	  
                      if(!nh.hilera.equals("Hilera:E")){ 
                           actual.info.ciclo();/*mueve la hilera*/}
                      
                      else{    
                          actual.info.cicloCircular();/*la hilera E recorre un ciclo circular*/}
                     ...
                     for(int i=0;i<nave.balas.size();i++){
                       ...
                       nave.balas.get(i).setY(900);}
                        ...
                        nh.setX(900); nh.setY(900);
                         ...
                         etiquetaNivel.setX(900); etiquetaNivel.setY(900);
                           puntaje.setX(900);
                       crearComponentesJuego();} /*comienza nuevo nivel....*/ }
             Thread.sleep(60);  }//hilo para dormir
            catch(InterruptedException e){System.out.println(e);}
              repaint();} }
    
/**
 *Este método se encarga de lo relacionado con el disparo de los invasores, esto quiere decir que se verifica que la bala esté en un rango de posición de algún invasor esto para poder destruirlo, y se destruye. También se comprueba si es jefe y si lo es se aplica la función o regla especÌfica para cada hilera.     
 */
    public void seDisparaAInvasor()  { 
        Nodo actual=invasores.raiz;
 while(actual!=null){
    
          if(listaActual.equals("B")){ contJefe=0;//Intercambia jefe con posicion aleatoria
                int a=  (int)(Math.random()*(invasores.cantidad) +1),cont3=0;//aleatoria
               Nodo aux=invasores.raiz;    
               
                while(aux!=null ){  cont3++;//busca nodo de posicion aleatoria
                if(cont3==a){break;}
                      aux=aux.sig;  } 
                  ...
                  if(Integer.parseInt(actual.info.getId().charAt(0)+"")!= a-1){
                       ...
                       actual .info.setX(900);
                          actual .info.setY(900);invasores.borrarNodo(actual.info.getX(), actual.info.getY());break;}
                   ...
                   else{System.out.println("intercambio al mismo jefe");//Pasa si el aletorio cae en el mismo jefe
                   
                     if(actual.sig!=null){ 
                         invasores.intercambiar(Integer.parseInt(actual.info.id.charAt(0)+""), Integer.parseInt(actual.sig.info.id.charAt(0)+""));
                         float x=  actual.info.getX();   float y=  actual.info.getY();
                   ...
                   if(actual.ant!=null){ 
                       invasores.intercambiar(Integer.parseInt(actual.info.id.charAt(0)+""), Integer.parseInt(actual.ant.info.id.charAt(0)+""));
                        float x=  actual.info.getX();   float y=  actual.info.getY();
                   ...
                   if(actual.ant==null&&actual.sig==null){ 
                       actual .info.setX(900);
                          actual .info.setY(900);invasores.borrarNodo(actual.info.getX(), actual.info.getY());break;};}   invasores.imprimir();               
          }
...
if( (nave.balas.get(i).getY()<actual.info.getY()+50&&nave.balas.get(i).getY()>actual.info.getY())&&(nave.balas.get(i).getX()>=actual.info.getX()   &&(nave.balas.get(i).getX()<actual.info.getX()+50))&&contDisparos==0&&!listaActual.equals("D")){ 
            nave.balas.get(i).setX(900);
            nave.balas.get(i).setY(900);
             invasores.imprimir();
  ...
  if(contJefe==3&&listaActual.equals("B")){contJefe= 0;
         invasores.borrarNodo((float)actual.info.getX(),(float)actual.info.getY()); 
          actual.info.setX(900);
         actual.info.setY(900);}
    
         if( contJefe==3&&listaActual.equals("A")){contJefe= 0;
              Nodo aux=invasores.raiz;
             while(aux!=null){ 
         ...
         if( contJefe==3&&listaActual.equals("C")){ contJefe=0;
           int a,cont3=0;
               Nodo aux=invasores.raiz;    
               

               while(true){a= (int)(Math.random()*(invasores.cantidad) +1); //Verifica que NO caiga en el mismo jefe
             ...
             if(Integer.parseInt(actual.info.getId().charAt(0)+"")!= a&&(actual.sig!=null||actual.ant!=null)){
                      float x=  aux.info.getX();   float y=  aux.info.getY();
                   invasores.intercambiar(Integer.parseInt(actual.info.id.charAt(0)+""),    Integer.parseInt(aux.info.id.charAt(0)+"")); 
                   
                   actual.info.setX(x); actual.info.setY(y);
                   aux.info.setX(900);aux.info.setY(900);
                   invasores.borrarNodo(aux.info.getX(), aux.info.getY());invasores.acomodarPorId(); 
                             break;}; }                                  
         
         
         
           if(contJefe==3&&listaActual.equals("E")){contJefe=0;
            invasores.borrarNodo((float)actual.info.getX(),(float)actual.info.getY()); 
              actual.info.setX(900);
         actual.info.setY(900);this.acomodarHilera();}  }
       
else{ // SI NO es jefe
            invasores.borrarNodo((float)actual.info.getX(),(float)actual.info.getY()); 
          actual.info.setX(900);
         actual.info.setY(900); break; } 
         puntos+=5;acomodarHilera();}  
        
        
   if( (nave.balas.get(i).getY()<actual.info.getY()+50&&nave.balas.get(i).getY()>actual.info.getY())&&(nave.balas.get(i).getX()>=actual.info.getX()   &&(nave.balas.get(i).getX()<actual.info.getX()+50)) &&listaActual.equals("D")){ 
      this.contDisparos=1;
       this.contDisparosHileraD[actual.info.resistencia-1]+=contDisparos; 
     nave.balas.get(i).setX(900); nave.balas.get(i).setY(900);nave.balas.remove(i);
     
     if(this.contDisparosHileraD[actual.info.resistencia-1]==actual.info.resistencia ){
      actual.info.setX(900);    actual.info.setY(900);
  ...
  if(actual.info.getY()==nave.getY()-40){//Si hilera llega a la nave gameover
      ...
      this.GuardarEstadisticas(this.nombreJug, puntos);System.exit(0); }
...
escribir.write("\r\n");
escribir.close(); /*cierra archivo txt*/}
